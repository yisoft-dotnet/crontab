	...
	public class FilterTest
	{
		...
		public void SpecificFilterTest()
		{
			...
			foreach (var test in tests)
			{
				...
				Assert.AreEqual(test.isMatch, method.IsMatch(test.output), "Is {0} a match to specific {1} of {2}?", test.output,
					Enum.GetName(typeof(CrontabFieldKind), test.kind),
					test.specific);
			}
		}
		...
		public void StepFilterTest()
		{
			var tests = new[]
			{
				...
				new {start = 5, step = 3, input = new DateTime(2016, 1, 2), isMatch = false},
		...
		public void BlankDayOfMonthOrWeekFilterInvalidState()
		{
			...
			foreach (var val in values)
				Assert2.Throws<CrontabException>(() => new BlankDayOfMonthOrWeekFilter(val),
					"Ensure BlankDayOfMonthOrWeekFilter can't be instantiated with <{0}>",
					Enum.GetName(typeof(CrontabFieldKind), val));
		...
		public void LastDayOfMonthFilterWorks()
		{
			...
			foreach (var pair in tests) Assert.AreEqual(pair.Value, method.IsMatch(pair.Key), "Is {0} the last day of the month");
		}
		...
		public void LastDayOfMonthFilterInvalidState()
		{
			...
			foreach (var type in values)
				Assert2.Throws<CrontabException>(() => new LastDayOfMonthFilter(type), "Ensure LastDayOfMonthFilter can't be instantiated with <{0}>",
		...
		{
			var values = Enum.GetValues(typeof(CrontabFieldKind))
				.Cast<CrontabFieldKind>()
				.Where(x => x != CrontabFieldKind.DayOfWeek);

			foreach (var type in values)
				Assert2.Throws<CrontabException>(() => new LastDayOfWeekInMonthFilter(0, type), "Ensure LastDayOfWeekInMonthFilter can't be instantiated with <{0}>",
					Enum.GetName(typeof(CrontabFieldKind), type));
		}
		...
		public void LastWeekdayOfMonthFilterTest()
		{
			...
			foreach (var test in tests) Assert.IsTrue(method.IsMatch(test.output), "Is {0} the last week day in a month");
		}
		...
		public void NearestWeekdayFilterInvalidState()
		{
			...
			foreach (var type in values)
				Assert2.Throws<CrontabException>(() => new NearestWeekdayFilter(1, type), "Ensure NearestWeekdayFilter can't be instantiated with <{0}>",
					Enum.GetName(typeof(CrontabFieldKind), type));
		...
		public void SpecificDayOfWeekInMonthFilterInvalidState()
		{
			...
			foreach (var type in values)
				Assert2.Throws<CrontabException>(() => new SpecificDayOfWeekInMonthFilter(0, 1, type),
					"Ensure SpecificDayOfWeekInMonthFilter can't be instantiated with <{0}>",
					Enum.GetName(typeof(CrontabFieldKind), type));
