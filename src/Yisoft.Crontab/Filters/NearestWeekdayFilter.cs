...
namespace Yisoft.Crontab.Filters
{
	...
	public class NearestWeekdayFilter : ICronFilter
	{
		...
		public NearestWeekdayFilter(int specificValue, CrontabFieldKind kind)
		{
			...
			if (kind != CrontabFieldKind.Day) throw new CrontabException(string.Format("<{0}W> can only be used in the Day field.", specificValue));
		...
		public override string ToString() { return string.Format("{0}W", SpecificValue); }
	}
}
