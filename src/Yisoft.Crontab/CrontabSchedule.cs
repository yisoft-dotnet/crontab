...
namespace Yisoft.Crontab
{
	public class CrontabSchedule
	{
		...
		public override string ToString()
		{
			...
			if (Format == CronStringFormat.WithSeconds || Format == CronStringFormat.WithSecondsAndYears) _JoinFilters(paramList, CrontabFieldKind.Second);

			_JoinFilters(paramList, CrontabFieldKind.Minute);
			_JoinFilters(paramList, CrontabFieldKind.Hour);
			_JoinFilters(paramList, CrontabFieldKind.Day);
			_JoinFilters(paramList, CrontabFieldKind.Month);
			_JoinFilters(paramList, CrontabFieldKind.DayOfWeek);

			if (Format == CronStringFormat.WithYears || Format == CronStringFormat.WithSecondsAndYears) _JoinFilters(paramList, CrontabFieldKind.Year);

			return string.Join(" ", paramList.ToArray());
		}

		public DateTime GetNextOccurrence(DateTime baseValue) { return GetNextOccurrence(baseValue, DateTime.MaxValue); }

		public DateTime GetNextOccurrence(DateTime baseValue, DateTime endValue) { return _InternalGetNextOccurence(baseValue, endValue); }
		...
		private DateTime MinDate(DateTime newValue, DateTime endValue) { return newValue >= endValue ? endValue : newValue; }

		private DateTime _InternalGetNextOccurence(DateTime baseValue, DateTime endValue)
		{
			...
			var newMinutes = Increment(minuteFilters, newValue.Minute, firstMinuteValue, out overflow);

			newValue = new DateTime(newValue.Year, newValue.Month, newValue.Day, newValue.Hour, newMinutes,
			...
			var newHours = Increment(hourFilters, newValue.Hour, firstHourValue, out overflow);
			...
			if (!overflow && !IsMatch(newValue))
			{
				newValue = new DateTime(newValue.Year, newValue.Month, newValue.Day, firstHourValue, firstMinuteValue,
					firstSecondValue);
				overflow = true;
			}
			...
			while (!(IsMatch(newValue, CrontabFieldKind.Day) && IsMatch(newValue, CrontabFieldKind.DayOfWeek) &&
			         IsMatch(newValue, CrontabFieldKind.Month) && (!isYearFormat || IsMatch(newValue, CrontabFieldKind.Year))))
			{
		...
		public bool IsMatch(DateTime value) { return Filters.All(fieldKind => fieldKind.Value.Any(filter => filter.IsMatch(value))); }
		...
		private void _JoinFilters(List<string> paramList, CrontabFieldKind kind)
		{
			paramList.Add(
				string.Join(",", Filters
					.Where(x => x.Key == kind)
		...
		{
			var monthSingle = _GetSpecificFilters(filters, CrontabFieldKind.Month);
			var daySingle = _GetSpecificFilters(filters, CrontabFieldKind.Day);

			if (monthSingle.Any()
			    && monthSingle.All(x => x.SpecificValue == 2) && daySingle.Any() &&
			    daySingle.All(x => x.SpecificValue == 30 || x.SpecificValue == 31)) throw new CrontabException("Nice try, but February 30 and 31 don't exist.");
		}
		...
		{
			if (string.IsNullOrWhiteSpace(cron)) throw new CrontabException("The provided cron string is null, empty or contains only whitespace");
			...
			if (instructions.Length > expectedCount) throw new CrontabException(string.Format("The provided cron string <{0}> has too many parameters", cron));
			if (instructions.Length < expectedCount) throw new CrontabException(string.Format("The provided cron string <{0}> has too few parameters", cron));
		...
		private static List<ICronFilter> _ParseField(string field, CrontabFieldKind kind)
		{
			try
			{
				return field.Split(',').Select(filter => _ParseFilter(filter, kind)).ToList();
			}
			catch (Exception e)
			{
		...
		private static ICronFilter _ParseFilter(string filter, CrontabFieldKind kind)
		{
			...
			try
			{
				...
				if (newFilter.StartsWith("L") && kind == CrontabFieldKind.Day)
				{
					...
					if (newFilter == "W") return new LastWeekdayOfMonthFilter(kind);

					return new LastDayOfMonthFilter(kind);
				}

				if (newFilter == "?") return new BlankDayOfMonthOrWeekFilter(kind);

				var firstValue = _GetValue(ref newFilter, kind);

				if (string.IsNullOrEmpty(newFilter))
				{
					if (kind == CrontabFieldKind.Year) return new SpecificYearFilter(firstValue, kind);
				...
				switch (newFilter[0])
				{
					...
					case '#':
					{
						...
						if (!string.IsNullOrEmpty(newFilter)) throw new CrontabException(string.Format("Invalid filter '{0}'", filter));

						return new SpecificDayOfWeekInMonthFilter(firstValue, secondValue, kind);
					}
					default:
						if (newFilter == "L" && kind == CrontabFieldKind.DayOfWeek) return new LastDayOfWeekInMonthFilter(firstValue, kind);
						else if (newFilter == "W" && kind == CrontabFieldKind.Day) return new NearestWeekdayFilter(firstValue, kind);
		...
		private static int _GetValue(ref string filter, CrontabFieldKind kind)
		{
			...
			if (string.IsNullOrEmpty(filter)) throw new CrontabException("Expected number, but filter was empty.");
			...
			for (i = 0; i < filter.Length; i++) if (isDigit && !char.IsDigit(filter[i]) || isLetter && !char.IsLetter(filter[i])) break;
			...
			if (replaceVal != null && replaceVal.Count == 1)
			{
				...
				if (filter.Length == i && filter.EndsWith("L") && kind == CrontabFieldKind.DayOfWeek) missingFilter = "L";
